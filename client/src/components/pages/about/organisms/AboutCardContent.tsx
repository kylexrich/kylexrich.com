...
const CardTag: React.FC<{ color: string; borderColor: string; text: string }> = ({color, borderColor, text}) => (
    <Tag size="sm" borderRadius="md" px={2} py={1} bg="transparent" border="1px solid" borderColor={borderColor}
         color={color}>
        {text}
    </Tag>
);
    ...
    return (
        <>
            <Flex justifyContent="space-between" alignItems="flex-start">
                <Flex>
                    <AboutCardImage logoRef={useColorModeValue(props.imageRefs.light, props.imageRefs.dark)}
                                    title={props.title}/>
                    <Stack spacing={2} pl={3} align="left">
                        <Heading textAlign="left" fontSize="xl"
                                 color={props.isHovered && props.hasModal ? titleColor : props.textColor}>
                            {props.title}
                        </Heading>
                        {props.subtitle && (
                        ...
                        <Text fontSize="sm" align="left" color={props.subTextColor} mt={1} mb={2}
