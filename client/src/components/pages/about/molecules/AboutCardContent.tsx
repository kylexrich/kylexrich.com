import React from "react";
import { Flex, Stack, Text, Box, Tag } from "@chakra-ui/react";
import AboutCardImage from "../atoms/AboutCardImage";
import AboutCardHeading from "../atoms/AboutCardHeading";
import { IGenericAboutCardDetail } from "../../../../config/about/IGenericAboutDetail";
import { ThemeProps } from "./AboutCard";

import SkillTags from "./SkillTags";

const CardTag: React.FC<{ color: string; borderColor: string; text: string }> = ({ color, borderColor, text }) => (
  <Tag
    size="sm"
    borderRadius="md"
    px={2}
    py={1}
    bg="transparent"
    border="1px solid"
    borderColor={borderColor}
    color={color}
  >
    {text}
  </Tag>
);

const DateText: React.FC<{ dateText: string; subTextColor: string }> = ({ dateText, subTextColor }) => (
  <Text fontSize="sm" color={subTextColor} w={"142px"}>
    {dateText}
  </Text>
);

type CardContentProps = IGenericAboutCardDetail & ThemeProps;
const AboutCardContent: React.FC<CardContentProps> = (props) => {
  return (
    <>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex>
          <AboutCardImage logoRef={props.logoRef} title={props.title} />
          <Stack spacing={2} pl={3} align="left">
            <AboutCardHeading title={props.title} textColor={props.textColor} />
            {props.subtitle && (
              <Text textAlign="left" fontSize="sm" color={props.subTextColor}>
                {props.subtitle}
              </Text>
            )}
            <Text
              fontSize="sm"
              align="left"
              color={props.subTextColor}
              mt={1}
              mb={2}
              display={{ base: "none", md: "block" }}
            >
              {props.shortDescription}
            </Text>
          </Stack>
        </Flex>
        <Stack display={{ base: "none", md: "flex" }} direction="row" spacing={3} alignItems="center">
          {props.type && (
            <>
              <CardTag color={props.textColor} borderColor={props.skillColor} text={props.type} />
              <Box height="15px" width="1px" bg={props.subTextColor} my={1}></Box>
            </>
          )}
          <DateText dateText={props.dateText} subTextColor={props.subTextColor} />
        </Stack>
      </Flex>
      <Stack mt={1} display={{ base: "flex", md: "none" }} direction="row" spacing={3} alignItems="center">
        <DateText dateText={props.dateText} subTextColor={props.subTextColor} />
        {props.type && (
          <>
            <Box height="15px" width="1px" bg={props.subTextColor} my={1}></Box>
            <CardTag color={props.textColor} borderColor={props.skillColor} text={props.type} />
          </>
        )}
      </Stack>
      <SkillTags
        skills={props.skills}
        skillColor={props.skillColor}
        textColor={props.textColor}
        subTextColor={props.subTextColor}
      />
    </>
  );
};

export default AboutCardContent;
